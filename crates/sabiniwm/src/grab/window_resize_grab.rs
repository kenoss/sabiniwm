...
pub(super) struct WindowResizeGrab {
    ...
    pub left_top: Point<i32, Logical>,
}
...
mod pointer_grab {
    ...
    impl smithay::input::pointer::PointerGrab<SabiniwmState> for WindowResizeGrab {
        fn motion(
            &mut self,
            ...
            const MINIMUM_SIZE: MySize = MySize { w: 100, h: 100 };

            handle.motion(state, None, event);

            let size = event.location.to_i32_round() - self.left_top;
            let w = size.x.max(MINIMUM_SIZE.w);
            let h = size.y.max(MINIMUM_SIZE.h);
            let size = Size::from((w, h));
            state
                .inner
                .view
                .update_float_window_with(self.window_id, |fw| {
                    fw.geometry.size = size;
                });
